...
try:
    from diff_gaussian_rasterization_wda import (
...
class GSLayer(nn.Module):
    ...
    def forward(
        self, x, pts, x_fine=None, gs_raw_attr=None, ret_raw=False, vtx_sym_idxs=None
    ):
        ...
        for k in self.attr_dict:
            ...
            if vtx_sym_idxs is not None and k in [
                "shs",
                "scaling",
                "opacity",
                "rotation",
            ...
            if (
                self.pred_res
                and (not self.fix_opacity or k != "opacity")
                and (not self.fix_rotation or k != "rotation")
            ):
...
class GS3DRenderer(nn.Module):
    ...
    def forward_single_view(
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            raster_ret = rasterizer(
                ...
                colors_precomp=(
                    colors_precomp.float() if colors_precomp is not None else None
                ),
    ...
    def animate_gs_model(
        self, gs_attr: GaussianModel, query_points, flame_data, debug=False
    ):
        """
        query_points: [N, 3]
        """
    ...
    def forward_gs_attr(
        self, x, query_points, flame_data, debug=False, x_fine=None, vtx_sym_idxs=None
    ):
        """
        ...
        gs_attr: GaussianModel = self.gs_net(
            x, query_points, x_fine, vtx_sym_idxs=vtx_sym_idxs
        )
        return gs_attr
    ...
    def forward_gs(
        ...
        **kwargs,
        ...
        # query_gs_features is gs_hidden_features 
        query_gs_features, query_points, flame_data = self.query_latent_feat(
    ...
    def forward_res_refine_gs(
        ...
        **kwargs,
        ...
        gs_model_list = []
        for b in range(batch_size):
            gs_model = self.gs_refine_net(
                query_gs_features[b],
                query_points[b],
                x_fine=None,
                gs_raw_attr=gs_raw_attr_list[b],
            )
    ...
    def forward_animate_gs(
        ...
        for b in range(batch_size):
            ...
            animatable_gs_model_list: list[GaussianModel] = self.animate_gs_model(
                gs_model,
                query_pt,
                self.get_single_batch_smpl_data(flame_data, b),
                debug=debug,
            )
    ...
    def project_single_view_feats(self, img_vtx_ids, feats, nv, inter_feat=True):
        ...
        if h != ih or w != iw:
            if inter_feat:
                ...
                vtx_ids = (
                    rearrange(vtx_ids, "b (c h) w k -> (b k) c h w", c=1)
                    .long()
                    .squeeze(1)
                )
            else:
                vtx_ids = (
                    torch.nn.functional.interpolate(
                        rearrange(vtx_ids, "b (c h) w k -> (b k) c h w", c=1).float(),
                        (ih, iw),
                        mode="nearest",
                    )
                    .long()
    ...
    def forward(
        ...
        **kwargs,
        ...
        gs_model_list, query_points, flame_data, query_gs_features = self.forward_gs(
            gs_hidden_features,
            query_points,
...
def test_head():
    ...
    dataset = VideoHeadDataset(
        ...
        meta_path=meta_path,
        sample_side_views=7,
        render_image_res_low=512,
        render_image_res_high=512,
        render_region_size=(512, 512),
    ...
    for k, v in out.items():
        ...
        for b_idx in range(len(v)):
            ...
            for v_idx in range(v.shape[1]):
                ...
                if "normal" in k:
                    img = (
                        (v[b_idx, v_idx].permute(1, 2, 0).detach().cpu().numpy() + 1.0)
                        / 2.0
                        * 255
                    ).astype(np.uint8)
                else:
                ...
                if "mask" in k:
                    ...
                    blend_img = (
                        (
                            render_images[v_idx].transpose(1, 2, 0) * 255 * 0.5
                            + np.tile(img, (1, 1, 3)) * 0.5
                        )
                        .clip(0, 255)
                        .astype(np.uint8)
                    )
